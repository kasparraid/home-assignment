...
describe('DeckController (integration)', function () {
  ...
  describe('findById()', function () {
    it('should validate deck id is valid uuid', async () => {
      ...
      // then
      expect(result.body.error.message).to.eql(
        'Provided id 123 is not a valid UUID',
      );
    });
    ...
    it('should validate deck exists', async () => {
      ...
      // then
      expect(result.body.error.message).to.eql(
        `Entity not found: Deck with id "${deckId}"`,
      );
    });
    ...
    it('should return a deck with some cards', async () => {
      ...
      // then
      const expected = givenDeckDto(deck, {
        remaining: 1,
        cards: [givenCardDto(card)],
      });
      expect(toJSON(result.body)).to.eql(toJSON(expected));
    });
  });
  ...
  describe('draw()', function () {
    ...
    it('should validate there are enough cards to draw', async () => {
      ...
      // then
      expect(result.body.error.message).to.eql(
        'Invalid count 5 to draw, 0 cards remaining',
      );
    });
    ...
    it('should draw multiple cards', async () => {
      // given
      const deck = await givenCreatedDeck(deckRepository);
      const firstCard = await givenCreatedCard(cardRepository, {
        deckId: deck.id,
      });
      const secondCard = await givenCreatedCard(cardRepository, {
        deckId: deck.id,
